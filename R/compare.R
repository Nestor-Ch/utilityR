...
compare_columns <- function(clean_data, raw_data, id_col, is.loop=F, columns_to_check, issue){
  ...
  if(length(diff_cols)>0){
    ...
    comparison_results <- clean_data %>%
      dplyr::select(uniqui, !!rlang::sym(id_col), dplyr::all_of(columns_to_check)) %>%
  ...
  comparison_results <- comparison_results %>% dplyr::select(!!rlang::sym(id_col), loop_index, variable, old.value, new.value, issue)
